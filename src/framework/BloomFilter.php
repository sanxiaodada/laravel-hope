...
class BloomFilter
{
    ...
    {
        $pipeline = Hope::getRedis()->multi();
        foreach ($this->hashFunction as $function) {
            $hash = $this->$function($value);
            $pipeline->setBit($this->prefix . $key, $hash, 1);
        }
        $pipeline->exec();
    }
    ...
    public function exists(string $key, string $value): bool
    {
        ...
        $len = strlen($value);
        foreach ($this->hashFunction as $function) {
            $hash = $this->$function($value, $len);
            $pipeline->getBit($this->prefix . $key, $hash);
        }
        $res = $pipeline->exec();
        foreach ($res as $bit) {
            if ($bit == 0) {
